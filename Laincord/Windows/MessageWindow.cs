...
namespace Laincord.Windows
{
    public class MessageWindow : IDisposable
    {
        ...
        public const uint WM_LAINCORD_CLOSE_FOR_REINSTALLATION = WM_APP + 26;

        // This name should never change, because rudimentary IPC methods (i.e. the installer) are
        // dependent on it.
        private static readonly string s_windowClass = "Laincord_MessageWindow_" + 
        ...
        private nint WndProc(HWND hWnd, uint uMsg, nint wParam, nint lParam)
        {
            switch (uMsg)
            {
                case WM_LAINCORD_CLOSE_FOR_REINSTALLATION:
                {
