...
namespace Laincord
{
    public partial class App : Application
    {
        ...
        private void StartLaincordMain()
        {
            ...
            _taskbarPresences = new()
                {
                { UserStatus.Online,        new BitmapImage(new Uri("pack://application:,,,/Laincord;component/Resources/Tray/Active.ico")) },
                { UserStatus.Idle,          new BitmapImage(new Uri("pack://application:,,,/Laincord;component/Resources/Tray/Idle.ico")) },
                { UserStatus.DoNotDisturb,  new BitmapImage(new Uri("pack://application:,,,/Laincord;component/Resources/Tray/Dnd.ico")) },
                { UserStatus.Invisible,     new BitmapImage(new Uri("pack://application:,,,/Laincord;component/Resources/Tray/Offline.ico")) },
                { UserStatus.Offline,       new BitmapImage(new Uri("pack://application:,,,/Laincord;component/Resources/Tray/Offline.ico")) },
            };

            var assembly = Assembly.GetExecutingAssembly();
            string resourceName = "Laincord.Scenes.Scenes.xml";
            ...
            if (tokenFound)
            {
                ...
                catch (NotSupportedException)
                {
                    ...
                    // for anyone who wants to try and get Laincord running on non-Windows operating
                    // systems. It might also show up on ancient versions of Windows that are modified
                    // enough to run Laincord but not enough to support the data encryption API.
            ...
            if (tokenFound)
            {
                Task.Run(async () =>
                {
                    LaincordLoginStatus success = await BeginLogin(token);
                    if (success != LaincordLoginStatus.Success)
                    {
        ...
        public async Task<LaincordLoginStatus> BeginLogin(string givenToken, bool save = false, UserStatus? status = null)
        {
            Discord.Client = new(new()
            {
                Token = givenToken,
            ...
            {
                return LaincordLoginStatus.Unauthorized;
            }
            catch (BadRequestException)
            {
                return LaincordLoginStatus.BadRequest;
            }
            catch (ServerErrorException)
            {
                return LaincordLoginStatus.ServerError;
            }
            ...
            return LaincordLoginStatus.Success;
        }
