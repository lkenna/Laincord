...
Unicode true
Name "Laincord"
Outfile "../Output/laincord-setup.exe"
...
# Required for detecting running Laincord processes and closing them:
!define LAINCORD_APP_GUID_NOBRACE "8231C4FA-AD94-487A-BDBF-A936306AE009"
!define LAINCORD_APP_GUID "{${LAINCORD_APP_GUID_NOBRACE}}"
!define LAINCORD_MESSAGE_WINDOW_CLASS "Laincord_MessageWindow_${LAINCORD_APP_GUID_NOBRACE}"
!define WM_LAINCORD_CLOSE_FOR_REINSTALLATION 32794 # WM_APP + 26

!define UNINSTALL_KEY "Software\Microsoft\Windows\CurrentVersion\Uninstall\Laincord"

VIProductVersion "${LAINCORD_VERSION}.0"
...
!define LEGACY_INNO_SETUP_INSTALL_PATH "$LOCALAPPDATA\Laincord"
!define LEGACY_INNO_SETUP_UNINSTALLER "${LEGACY_INNO_SETUP_INSTALL_PATH}\unins000.exe"
!define LEGACY_INNO_SETUP_CONFIG_FILE "Laincord.json"

${Using:StrFunc} StrCase
${Using:StrFunc} UnStrCase

Function CreateDesktopShortcut
    CreateShortcut "$desktop\Laincord.lnk" "$instdir\Laincord.exe"
FunctionEnd
...
!define MUI_ICON "../Laincord/Icons/MainWnd.ico"
!define MUI_UNICON "../Laincord/Icons/MainWnd.ico"
...
!define MUI_FINISHPAGE_RUN "$instdir\Laincord.exe"
...
Function InnoSetupMigration
    ...
    DetailPrint "Uninstalling old version of Laincord..."
    
    # Old Inno Setup versions of Laincord don't open a mutex to notify if they're running, so we just kill
    # any running process named "laincord.exe"
    Push $1
    ${nsProcess::KillProcess} "Laincord.exe" $1
    Pop $1
    # Above command pops $1 internally
    
    DetailPrint "Ensured that Laincord.exe is not running."
    
    # Because the Inno Setup version exists, we're going to make two transformations.
    #    1. Laincord v0.1 (versions using Inno Setup) were always installed to local appdata
    #       and stored their configuration alongside the program in Laincord.json. We want
    ...
    CreateDirectory "$APPDATA\Laincord" # Ensure that the application data directory exists.
    IfErrors _EnsureAppDataDirFailure _EnsureAppDataDirSuccess
_EnsureAppDataDirFailure:
    DetailPrint "Failed to create roaming application data folder '$APPDATA\Laincord'."
_EnsureAppDataDirSuccess:
    Rename "${LEGACY_INNO_SETUP_INSTALL_PATH}\${LEGACY_INNO_SETUP_CONFIG_FILE}" "$APPDATA\Laincord\config.json"
    IfErrors _MoveConfigError
    DetailPrint "Moved user configuration file to '$APPDATA\Laincord\config.json'."
    goto _MoveConfigSuccess
_MoveConfigError:
    DetailPrint "Failed to move user configuration file to '$APPDATA\Laincord\config.json'."
...
_MoveConfigSuccess:
    ...
    DetailPrint "Successfully uninstalled old version of Laincord."
...
Function ${prefix}IsApplicationMutexOpen
    ...
    !if `${prefix}` == "un."
        ${UnStrCase} $R0 ${LAINCORD_APP_GUID_NOBRACE} "L"
    !else
        ${StrCase} $R0 ${LAINCORD_APP_GUID_NOBRACE} "L"
    !endif
...
Function ${prefix}EnsureApplicationClosed
    ...
    DetailPrint "Making sure that there isn't any Laincord process running..."
    ...
    FindWindow $R0 "${LAINCORD_MESSAGE_WINDOW_CLASS}"
    ...
    DetailPrint "Sending quit message to Laincord message window..."
    SendMessage $R0 ${WM_LAINCORD_CLOSE_FOR_REINSTALLATION} 0 0
...
_ExitSleepLoop:
    Pop $R1
    DetailPrint "Failed to safely close the Laincord process despite our best efforts."
...
_LogFindMessageFailureAndForcefullyClose:
    DetailPrint "Failed to find the Laincord message window, \
however we know for a fact that the process is running."
    goto _ForcefullyClose
    
_ForcefullyClose:
    # If we ended up here, then we failed at any sort of graceful exit.
    # We will just kill any process named "Laincord.exe" and hope for the
    # best...
    DetailPrint "Forcefully closing any process named Laincord.exe."
    ${nsProcess::KillProcess} "Laincord.exe" $R0
...
# Vista or Windows 7 system, and thus wouldn't be able to run Laincord.
...
Section "Laincord" Laincord
    ...
    # Note that this function only handles NSIS-installer versions of Laincord. Legacy Inno Setup
    ...
    ${PLACE_FILE} /r /x *.pdb /x *.xml /x Laincord.json "..\Laincord\${LAINCORD_BIN_FOLDER}\*"
    
!ifdef LAINCORD_RC # We include PDBs for RC builds.
    ${PLACE_FILE} /r "..\Laincord\${LAINCORD_BIN_FOLDER}\*.pdb"
!else
...
_VcrtAlreadyInstalled:
    ...
    # Laincord requires the .NET runtime, of course.
    ...
    CreateShortCut "$SMPROGRAMS\Laincord.lnk" \
        "$InstDir\Laincord.exe"
    SetShellVarContext lastused
    
    # Create Uninstall entry
    SetRegView 64
    WriteRegStr SHCTX "${UNINSTALL_KEY}" \
                 "DisplayName" "Laincord"
    WriteRegStr SHCTX "${UNINSTALL_KEY}" \
                 "DisplayIcon" "$InstDir\Laincord.exe,0"
    WriteRegStr SHCTX "${UNINSTALL_KEY}" \
...
Section "Uninstall"
    ...
    # Delete shortcut
    SetShellVarContext all
    Delete "$SMPROGRAMS\Laincord.lnk"
    SetShellVarContext lastused

!if 0 # Currently does not work, so it's disabled for the Stability Test Release.
    # Since we want to be nice, we will only delete the desktop shortcut if we know it is Laincord.
    IfFileExists "$desktop\Laincord.lnk" 0 _NoDesktopShortcut
    ...
    System::Call "$3->${IPersistFile_Load}(t '$desktop\Laincord.lnk', i ${STGM_READ})i .r1"
